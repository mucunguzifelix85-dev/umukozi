    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-lg mx-auto bg-white rounded-3xl shadow-xl p-6">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 font-bold mt-1">{t.registerWorker}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-4">
            <p className="text-xs font-black text-blue-700 uppercase mb-2">💼 {t.skillsLabel} *</p>
            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
              {JOB_TYPES.map(job => (
                <button type="button" key={job}
                  onClick={() => toggleSkill(job, skills, setSkills)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
                    skills.includes(job) ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-300 hover:border-blue-400"
                  }`}>
                  {job}
                </button>
              ))}
          ...
          <div className="bg-orange-50 border-2 border-orange-200 rounded-2xl p-4">
            <p className="text-xs font-black text-orange-700 uppercase mb-2">⭐ {t.experienceLabel}</p>
            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto">
              {JOB_TYPES.map(job => (
                <button type="button" key={job}
                  onClick={() => toggleSkill(job, experiencedIn, setExperiencedIn)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
                    experiencedIn.includes(job) ? "bg-orange-500 text-white border-orange-500" : "bg-white text-gray-600 border-gray-300 hover:border-orange-400"
                  }`}>
                  {job}
                </button>
              ))}
