    ...
    return (
      <div className="min-h-screen bg-red-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm text-center">
          ...
          <h2 className="text-xl font-black text-red-600 mb-2">{t.payFirst}</h2>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow p-4 mb-6 flex gap-3">
          <select value={searchJob} onChange={e => setSearchJob(e.target.value)}
            className="flex-1 border-2 border-gray-200 rounded-xl p-3 font-bold focus:border-green-500 outline-none bg-white">
            <option value="">{t.searchByJob}</option>
            {JOB_TYPES.map(j => <option key={j} value={j}>{j}</option>)}
          </select>
          {searchJob && (
            <button onClick={() => setSearchJob("")}
              className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl font-bold text-gray-600">
              X Clear
            </button>
          )}
        </div>
        <div className="bg-white rounded-2xl shadow mb-3 px-4 py-2 flex justify-between items-center">
          <span className="font-black text-gray-700">{t.results}: <span className="text-green-600">{filtered.length}</span></span>
                ...
                <tbody>
                  {filtered.map((w, i) => (
                    <tr key={w.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="p-3 font-black text-gray-800">{w.fullName}</td>
                      <td className="p-3 font-bold text-green-700">{w.phoneNumber}</td>
                      <td className="p-3 font-bold text-gray-600">{w.location.district}</td>
                      <td className="p-3 font-bold text-gray-600">{w.location.sector}</td>
                      <td className="p-3 font-bold text-gray-600">{w.location.neighborhood}</td>
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          {w.skills.map(s => (
                            <span key={s} className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs font-bold">{s}</span>
                          ))}
                        </div>
                      </td>
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          {w.experiencedIn.map(s => (
                            <span key={s} className="bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full text-xs font-bold">{s}</span>
                          ))}
                        </div>
                      </td>
                    </tr>
